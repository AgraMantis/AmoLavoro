<template>

 <v-row>
   <v-img
  lazy-src="https://i.imgur.com/dlWtCAt.png"
  max-height="60"
  max-width="150"
  src="https://i.imgur.com/dlWtCAt.png"
></v-img>
   

  
  
  
  
    <v-col class="text-center">
      <v-btn
      class="ma-2"
      outlined
      large
      fab
      color="indigo"
    >
    <NuxtLink to="/storia">
      <v-avatar size="64">
          
        <img
        
          src="https://www.sardiniauniqueproperties.com/wp-content/uploads/2015/10/Square-Profile-Pic-1-2.jpg"
        
        >
      </v-avatar>
    </NuxtLink>  
    
      
    </v-btn>
          <v-btn
      class="ma-2"
      outlined
      large
      fab
      color="indigo"
    >
      <v-avatar size="64">
            
      <img
        
        src="http://www.classicaloasis.com/wp-content/uploads/2014/03/profile-square.jpg"
        
      >
    </v-avatar>
    </v-btn>
          <v-btn
      class="ma-2"
      outlined
      large
      fab
      color="indigo"
    >
      <v-avatar size="64">
            
      <img
        
        src="https://www.stepstherapy.com.au/wp-content/uploads/2018/10/Yazmin-profile-picture-square.jpg"
        
      >
    </v-avatar>
    </v-btn>
          <v-btn
      class="ma-2"
      outlined
      large
      fab
      color="indigo"
    >
      <v-avatar size="64">
            
      <img
        
        src="data:image/jpeg;base64,"
        
      >
    </v-avatar>
    </v-btn>
         <v-btn
      class="ma-2"
      outlined
      large
      fab
      color="indigo"
    >
      <v-avatar size="64">
            
      <img
        
        src="http://radekborkowski.pl/wp-content/uploads/2018/01/opulent-profile-square-07.jpg"
        
      >
    </v-avatar>
    </v-btn>
    <v-row>
    <v-card
    justify="end"
    class="my-4 "
    max-width="200"
    height="45"
    rounded="30"
    style="overflow:hidden"
    >

        <v-toolbar
        class="mx-auto"
        dense
        rounded="30"
        floating
        justify="end"
    >
      <v-text-field
        hide-details
        prepend-icon="mdi-magnify"
        single-line
      ></v-text-field>
    </v-toolbar>
    </v-card>
   
      <v-col cols="6">
        
      </v-col>
      <v-col cols="2">
        <v-select
           @change="cambia_lingua"
          :items="[
            'English','Español','Français','Italiano','ਪੰਜਾਬੀ',
        ]"
          menu-props="auto"
          label="Select"
          hide-details
          prepend-icon="mdi-map"
          single-line
        ></v-select>
      </v-col>
      <v-col cols="1">
      </v-col>
    </v-row>
  

  <v-card
    class="mx-auto my-4"
    color=#27004e
    max-width="700"
    outlined
    
  >
    <v-list-item three-line>
      <v-list-item-content>
        <div class="text-overline mb-4">
          {{ $t('history') }}
        </div>
        <v-list-item-title class="text-h5 mb-1">
          {{ $t('racconta') }}
        </v-list-item-title>
        <v-list-item-subtitle>{{ $t('potrebbe') }}</v-list-item-subtitle>
      </v-list-item-content>

          <v-avatar size="100">
            
      <img
        size="128"
        src="https://i.imgur.com/8j27HGM.png"
        
      >
    </v-avatar>
    </v-list-item>

    <v-card-actions>
       <v-row
    align="center"
    justify="end"
  >
      <v-btn
        rounded
        dark
        color="purple"
      >
        {{ $t('scrivi') }}
      </v-btn>
      </v-row>
    </v-card-actions>
  </v-card>
   
   
   <v-card
   
    class="mx-auto my-4"
    color=#20603d
    max-width="700"
    outlined
  >
    <v-list-item three-line>
      <v-list-item-content>
        <div class="text-overline mb-4">
          {{ $t('segnalazioni') }}
        </div>
        <v-list-item-title class="text-h5 mb-1">
          {{ $t('lavori_disonesti') }}
        </v-list-item-title>
        <v-list-item-subtitle>{{ $t('leggi_segnala') }}</v-list-item-subtitle>
      </v-list-item-content>

        <v-avatar  size="100" right>
            
      <img
       
        src="https://i.imgur.com/PWNxssX.jpg"
        
      >
      
    </v-avatar>
    </v-list-item>

    <v-card-actions>
       <v-row
    align="end"
    justify="end"
  >
      <v-btn
        rounded
        dark
        color="green"
      >
        {{ $t('segnala') }}
      </v-btn>
      </v-row>
    </v-card-actions>
  </v-card>

  <v-card
   
    class="mx-auto my-4"
    color=#3f888f
    max-width="700"
    outlined
  >
    <v-list-item three-line>
      <v-list-item-content>
        <div class="text-overline mb-4">
          {{ $t('avvocati') }}
        </div>
        <v-list-item-title class="text-h5 mb-1">
          {{ $t('ricevi_aiuto') }}
        </v-list-item-title>
        <v-list-item-subtitle>{{ $t('aiuto_gratuito') }}</v-list-item-subtitle>
      </v-list-item-content>

          <v-avatar size="100">
            
      <img
        size="128"
        src="https://i.imgur.com/RCEvNFJ.jpg"
        
      >
    </v-avatar>
    </v-list-item>

    <v-card-actions>
       <v-row
    align="end"
    justify="end"
  >
      <v-btn
        rounded
        dark
        color="cyan"
      >
        {{ $t('aiuto') }}
      </v-btn>
      </v-row>
    </v-card-actions>
  </v-card>

  <v-card
    class="mx-auto my-4"
    color=#f7d147
    max-width="700"
    outlined
  >
    <v-list-item three-line>
      <v-list-item-content>
        <div class="text-overline mb-4">
          {{ $t('posti') }} 
        </div>
        <v-list-item-title class="text-h5 mb-1">
          {{ $t('opportunita') }}
        </v-list-item-title>
        <v-list-item-subtitle>{{ $t('offerte_lavoro') }}</v-list-item-subtitle>
      </v-list-item-content>

          <v-avatar size="100">
            
      <img
        size="128"
        src="https://i.imgur.com/78mD5sJ.png"
        
      >
    </v-avatar>
    </v-list-item>

    <v-card-actions>
       <v-row
    align="center"
    justify="end"
  >
      <v-btn
        rounded
        dark
        color="yellow"
      >
        Scrivi
      </v-btn>
      </v-row>
    </v-card-actions>
  </v-card>

  <v-card
    class="mx-auto my-4"
    color=#ff6088
    max-width="700"
    outlined
  >
    <v-list-item three-line>
      <v-list-item-content>
        <div class="text-overline mb-4">
          {{ $t('info') }}
        </div>
        <v-list-item-title class="text-h5 mb-1">
          {{ $t('informati') }}
        </v-list-item-title>
        <v-list-item-subtitle>{{ $t('informazioni_lavoro') }}</v-list-item-subtitle>
      </v-list-item-content>

          <v-avatar size="100">
            
      <img
        size="128"
        src="https://i.imgur.com/9zzwdeN.png"
        
      >
    </v-avatar>
    </v-list-item>

    <v-card-actions>
       <v-row
    align="center"
    justify="end"
  >
      <v-btn
        rounded
        dark
        color="pink"
      >
        {{ $t('informati') }}
      </v-btn>
      </v-row>
    </v-card-actions>
  </v-card>
 
      <!--<img
        src="/v.png"
        alt="Vuetify.js"
        class="mb-5"
      >--->
      <blockquote class="blockquote">
        &#8220;AmoLavoro&#8221;
        <footer>
          <small>
            <em>&mdash;un progetto per gli ultimi che saranno i primi</em>
          </small>
        </footer>
      </blockquote>
      <NuxtLink to="/storia">
         <v-btn
      rounded
      color="purple"
      dark
      x-large
      
    >
      Accanna
    </v-btn>
    </Nuxtlink>
    
    
    </v-col>
  </v-row>
</template>

<script>
export default {
  name: 'InspirePage',
  methods:{
    cambia_lingua(value){
      console.log("ciao",value)
      if(value == "English"){
        this.$i18n.setLocale('en')
      }
      if(value == "Italiano"){
        this.$i18n.setLocale('it')
      }
      if(value == "Français"){
        this.$i18n.setLocale('fr')
      }
      if(value == "Español"){
        this.$i18n.setLocale('es')
      } 
      if(value == "ਪੰਜਾਬੀ"){
        this.$i18n.setLocale('pa')
      }     
    }
  }
}
</script>
